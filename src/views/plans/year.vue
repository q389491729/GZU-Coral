...
export default {
  ...
  methods: {
    ...
    async save(data, node, done, loading) {
      let obj = JSON.parse(JSON.stringify(this.needForm));
      ...
      try {
        ...
        this.$message.error("需求计划名称重复,请重新再试");
        loading();
      }
    },
    nodeClick(data) {
      // let index;
      // let o = JSON.parse(JSON.stringify(this.treeData))
      ...
      // o[index].is_show = true
      // // this.treeData[index].is_show = true
      // this.treeData = o
      data.is_show = true;
      this.isSelect = true;
    ...
    fetch() {
      getAllPlans("年度计划").then(res => {
        let data = res.data.data;
        this.treeData = data;
        if (data[0]) this.tableData = data[0].goodsneed;
        else this.tableData = [];
        this.isSelect = false;
      });
    },
